...
contract Handler is CommonBase, StdCheats, StdUtils {
    ...
    function sendToFallback(uint256 amount) public {
        amount = bound(amount, 0, address(this).balance);    //  i/p range in between 0 -> address(this).balance
        wethBalance += amount;                               //  after getting eth , increase the wethBalance 
        numCalls += 1;
    ...
    function deposit(uint256 amount) public {
        amount = bound(amount, 0, address(this).balance);   // i/p range 0 -> 100
        wethBalance += amount;
        numCalls += 1;
...
contract WETH_Handler_Based_Invariant_Tests is Test {
    ...
    function setUp() public {
        ...
        bytes4[] memory selectors = new bytes4[](3);  // sequences here
    ...
    function invariant_eth_balance() view public {
        assertGe(address(weth).balance, handler.wethBalance());  // assertGe (greater then equal to ...)
